...
const table_name = ref('Notifications')
const table_row_style = ref('notifications_table')
const table_headers = ref(['Datetime', 'Category', 'Message', 'Channel Name'])
const table_buttons = ref({'All':'all', 'Settlement':'settlement_action', 'Stripe':'stripe', 'User':'user_settings'})
...
const csrf_cookie = useCookie('csrf_access_token')
  ...
  fetch(`${config.flask_url}/api/user_dashboard/get_notifications/`, {
      method: 'post',
      mode: 'cors',
      credentials: 'include',
      headers: {
